...
export function NotePageHeader({ note }: { note: NoteRecord }) {
  return (
    <div className={cn("flex flex-col gap-3 pb-6")}>
      ...
      <div className={cn("flex flex-wrap gap-2", "text-sm text-muted-foreground")}>
        {note.tags?.map(tag => <span key={tag} className="text-xs text-muted-foreground">#{tag}</span>)}
      </div>
      <div className={cn("flex gap-2")}>
        <Badge>{note.status}</Badge>
        <Badge variant="outline">{note.difficulty}</Badge>
        <span className={cn("self-center text-xs text-muted-foreground")}>{note.date}</span>
      </div>
    </div>
  )
}
