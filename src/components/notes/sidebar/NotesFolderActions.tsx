...
export function NotesFolderActions(props: NotesFolderActionsProps) {
  return (
    <div className="absolute right-2 top-1/2 flex -translate-y-1/2 items-center gap-1 opacity-0 group-hover/folder:opacity-100">
      <Button variant="ghost" size="icon-xs" onClick={props.onEdit} className="hover:text-sky-500"><Pencil className="size-4" /></Button>
      <Button variant="ghost" size="icon-xs" onClick={props.onCreateNote} className="hover:text-emerald-500"><FilePlus className="size-4" /></Button>
      <Button variant="ghost" size="icon-xs" onClick={props.onCreateFolder} className="hover:text-amber-500"><FolderPlus className="size-4" /></Button>
