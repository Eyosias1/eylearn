...
import type { PerfFilter, SortKey, DateRange } from "@/components/plan/history/table/retrospective-full"
...
export function FilterBar({
  query, sort, subject, perf, dateRange, subjects, dueCount,
  onQuery, onSort, onSubject, onPerf, onDateRange,
}: Props) {
  const isDueActive = perf === "due"
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      ...
      <Select value={perf === "due" ? "all" : perf} onValueChange={(v) => onPerf(v as PerfFilter)}>
      ...
      <Button
        variant={isDueActive ? "default" : "outline"}
        size="sm"
        onClick={() => onPerf(isDueActive ? "all" : "due")}
        className={cn(
          // spacing
          "h-9 gap-1.5",
          // typography
      ...
      >
        ...
        {dueCount > 0 && (
          <span className={cn(
            ...
            "text-xs font-bold",
            // conditional
            isDueActive ? "bg-background/20 text-background" : "bg-destructive/10 text-destructive",
          )}>
            {dueCount}
          </span>
        )}
