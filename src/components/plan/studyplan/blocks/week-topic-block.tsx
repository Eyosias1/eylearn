...
export function WeekTopicBlock({ topic, selected, onClick, layout }: Props) {
  const col      = layout?.col ?? 0
  const colCount = layout?.colCount ?? 1
  const top      = ((timeToMinutes(topic.startTime) - 6 * 60) / 60) * WEEK_HOUR_PX
  const w        = 100 / colCount
  const l        = col * w

  const { duration, onMouseDown: onResizeDown } = useBlockResize(
    topic.durationMinutes, WEEK_HOUR_PX,
  ...
  return (
    <button
      ...
      className={cn(
        ...
        "transition-opacity",
        // conditional
        selected ? "opacity-100 ring-1 ring-foreground/30" : "opacity-85 hover:opacity-100",
      )}
    >
      <div className={cn("absolute inset-0 opacity-20", topic.subjectColor)} />
      <div className={cn("absolute left-0 top-0 bottom-0 w-0.5", topic.subjectColor)} />
      <div className="relative flex flex-col justify-start px-1.5 pt-0.5 h-full">
        <p className="text-xs font-semibold leading-tight truncate">{topic.name}</p>
        <p className="text-[11px] text-muted-foreground leading-tight">{topic.startTime} · {duration}m</p>
      </div>
      <div
