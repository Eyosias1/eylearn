...
import { SubjectColorPicker, SUBJECT_COLORS } from "@/components/library/subjects/subject-color-picker"
...
export function SubjectDialog({ open, subject, onClose }: Props) {
  const [prevDep, setPrevDep] = useState(`${subject?.id}-${String(open)}`)
  const [name,    setName]    = useState(subject?.name ?? "")
  const [color,   setColor]   = useState(subject?.color ?? SUBJECT_COLORS[0])
  const [loading, setLoading] = useState(false)

  const dep = `${subject?.id}-${String(open)}`
  if (dep !== prevDep) {
    setPrevDep(dep)
