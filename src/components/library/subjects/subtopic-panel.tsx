...
import { NotePicker } from "@/components/library/subjects/note-picker"
...
export function SubtopicPanel({ subtopic, topic, subject, onClose }: Props) {
  ...
  if (prevSubtopicId !== subtopic?.id) {
    setPrevSubtopicId(subtopic?.id)
    setPicking(false)
    setLinkedNotes([])
    if (subtopic?.id) getLinkedNotes(subtopic.id).then(setLinkedNotes)
  }
